...
def _check_condition(record: dict, condition: str) -> bool:
    ...
    if condition == "join_key_missing":
        return not record.get("trace_id") and not record.get("event_hash")
...
def _classify_execute_outcome(
    ...
    if direction == "LONG":
        ret_hit = thresholds.get("long_hit_return_t5", 0.02)
        alpha_hit = thresholds.get("long_hit_alpha_t5", 0.01)
        ret_miss = thresholds.get("long_miss_return_t5", -0.02)
        alpha_miss = thresholds.get("long_miss_alpha_t5", -0.01)
        ...
        ret_hit = thresholds.get("short_hit_return_t5", -0.02)
        alpha_hit = thresholds.get("short_hit_alpha_t5", -0.01)
        ret_miss = thresholds.get("short_miss_return_t5", 0.02)
        alpha_miss = thresholds.get("short_miss_alpha_t5", 0.01)
...
def _compute_monotonicity(
    ...
    min_sample = dq.get("min_bucket_sample_size", 10)
    min_total = dq.get("min_total_sample_size", 30)
