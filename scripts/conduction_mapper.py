...
from opportunity_score import PremiumStockPool
...
class ConductionMapper(EDTModule):
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.stock_pool = PremiumStockPool()
    ...
    def _ai_stock_candidates(
        self,
        semantic_output: Dict[str, Any] = None,
        sector_data: List[Dict[str, Any]] = None,
    ) -> List[Dict[str, Any]]:
        ai_stocks: List[Dict[str, Any]] = []
        ...
        for symbol in recommended:
            ...
            if stock_info is None:
                fallback_stock = self.stock_pool.get_stock(symbol)
                if fallback_stock is not None:
                    stock_info = {
                        "symbol": fallback_stock.symbol,
                        "sector": fallback_stock.sector,
                    }
    ...
    def _build_template_mapping(self, template: Dict[str, Any], sector_data: List[Dict[str, Any]], semantic_output: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        ai_stocks = self._ai_stock_candidates(semantic_output, sector_data)
        merged_candidates = []
        seen_symbols = set()
        for stock in ai_stocks + stock_candidates:
    ...
    def _policy_mapping(
        ...
        for stock in ai_stocks + stock_candidates:
            ...
            if symbol and symbol not in seen_symbols:
                ...
                seen_symbols.add(symbol)
        if not merged_candidates:
            merged_candidates = stock_candidates
    ...
    def execute(self, input_data: ModuleInput) -> ModuleOutput:
        ...
        if template and template.get("id") == "tariff_chain":
            ...
            mapping = self._policy_mapping(policy_intervention, sector_data, semantic_out)
